...
def HW9():
    ...
    return render_template('hw9.html', suppids=suppids)
...
@app.route('/chart')
def graphProducts():
    supplier_id = request.args.get('suppid', default=1, type=int)

    sql = "SELECT ProductName AS label, (UnitsInStock * UnitPrice) AS value FROM Products WHERE SupplierID=%s"
    cursor.execute(sql, (supplier_id,))
    products = cursor.fetchall()

    chartData = json.dumps(products)

    return render_template('ProductChart.html', chartData=chartData)
